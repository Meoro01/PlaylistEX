...
void load_data(Heap* heap, const char* filename, const char* type) {
    ...
    while (fgets(line, sizeof(line), file)) {
        ...
        if (strcmp(type, "book") == 0) {
            ...
            if (sscanf(line, "%[^-] - %[^-] - %[^\n]", song.title, song.artist, genres) == 3) {
                trim(song.title);
                trim(song.artist);
...
void recommend_songs(Heap* heap, const char* book_genre) {
    ...
    for (int i = 0; i < heap->song_count; i++) {
        int total_weight = 0; // 최종 가중치 합산 변수

        for (int g = 0; g < 2; g++) { // 각 장르에 대해 계산
            if (strlen(heap->songs[i].genres[g]) == 0) continue; // 빈 장르 스킵
            ...
            for (int j = 0; j < genre_weight_count; j++) {
                if (strcmp(book_genre, genre_weights[j].book_genre) == 0 &&
                    ...
                    total_weight += genre_weights[j].weight; // 매핑된 가중치 추가
                    matched = true;
                    break;
                }
            }
            ...
            if (!matched) {
                total_weight += 1; // 매핑되지 않은 장르 기본 가중치 추가
            }
        }

        heap->songs[i].weight = total_weight; // 최종 가중치 저장
